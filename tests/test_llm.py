...
class DummyGoodSim(BaseLLMClient):
    def _raw_generate(self, prompt, model=None, **kwargs):
        return json.dumps(
            {"stdout": "ok", "stderr": "", "exit_code": 0, "explanation": "ok"}
        )


def test_simulate_command_success():
    client = DummyGoodSim()
    res = client.simulate_command(
        "ls", fs={"type": "dir", "name": "/", "children": []}, bash_history=["pwd"]
    )
...
def test_simulate_command_parse_failure():
    client = DummyBad()
    res = client.simulate_command(
        "ls", fs={"type": "dir", "name": "/", "children": []}, bash_history=[]
    )
    assert res["exit_code"] == 1
    assert "sh: internal parse error" in res["stderr"]
        ...
        return json.dumps(
            {
                "type": "dir",
                "name": "root",
                "children": [{"type": "dir", "name": "logs", "children": []}],
            }
        )
...
def test_generate_scenario_filesystem_success():
    ...
    fs = client.generate_scenario_filesystem(
        description="IoT camera compromise", target_dir="/home/user"
    )
    assert fs["type"] == "dir"
